...
# commands in a single-threaded environment. It runs both commands with the 
...
measure_time_and_output() {
    ...
    for ((i=1; i<=num_iterations; i++)); do
        start_time=$(date +%s%N)
        eval taskset -c 0 ${command[@]} > "$output_file" 2>&1
        end_time=$(date +%s%N)
...
generate_random_file rf_1MB $(expr 256 \* 1024)
